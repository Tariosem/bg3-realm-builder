...
function MaterialPresetsMenu:SetupWorkspace(parent, ccaModPack)
    ...
    function refreshSelectedList()
        ...
        local emptyBox,addText = tailRow:AddCell():AddColorEdit("##dropPreset"), tailRow:AddCell():AddText("<- Drop here")
...
function MaterialPresetsMenu:RenderExportSettingPanel(parent, settings)
    ...
    modNameInput.OnChange = Debounce(10, function()
    ...
    authorNameInput.OnChange = Debounce(10, function()
...
function MaterialPresetsMenu:ImportFromFile(modName, version)
    ...
    local versionStr = type(version) == "table" and (tostring(version[1]) .. "." .. tostring(version[2]) .. "." .. tostring(version[3]) .. "." .. tostring(version[4])) or tostring(version)
...
function MaterialPresetsMenu:__exportToMod(modPack, progressCallback)
    ...
    local progress = 0
    local progressStep = 20 / (#matPresets)
    progressStep = math.floor(progressStep)
    local progressCompensation = 20 - (progressStep * (#matPresets))
    ...
    local function advance(step, message)
    ...
    if not suc then throwError("ExportToMod: Failed to save mod meta file at " .. mataFilePath) return end
    advance(20, "Saved mod meta file.")
    ...
    advance(20, "Saved localization file.")
    ...
    for _, preset in pairs(matPresets) do
        ...
        advance(progressStep, "Building material preset banks resource nodes...")
    end
    advance(progressCompensation, "Built material preset banks...")
    ...
    for presetType, bank in pairs(banks) do
        ...
        advance(3, "Saving material preset banks...")
    end
    advance(1, "Saved material preset banks...")
    ...
    for _, preset in pairs(matPresets) do
        ...
        advance(progressStep, "Building CCA presets definition...")
    end
    advance(progressCompensation, "Built CCA presets definition...")
    ...
    for presetType, def in pairs(ccaDefNode) do
        ...
        advance(3, "Saving CCA presets definition...")
    end
...
function MaterialPresetsMenu:SaveModCache(modName, authorName, description, version, matPresets, modInternalName, modUuid)
    ...
    if not suc then Warning("ExportToMod: Failed to save CCA mod cache file at " .. filePath) end
