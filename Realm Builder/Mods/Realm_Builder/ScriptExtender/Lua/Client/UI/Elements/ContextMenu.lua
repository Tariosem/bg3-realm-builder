...
--- @field AddItems fun(self: RB_ContextMenu, context: RB_ContextItem[], isFocus?: fun(): boolean)
...
function ContextMenuClass:AddItem(label, onClick, hint, image)
    ...
    if image and image ~= "" then
        ...
        img.ImageData.Size = RBUtils.ToVec2(36 * SCALE_FACTOR)
        tab.BordersInnerV = true
    end
    local innerCell = row:AddCell()
    local innerTable = innerCell:AddTable("InnerTable##" .. RBUtils.Uuid_v4(), 3) --[[@as ExtuiTable]]
    ...
    if hint and hint ~= "" then
        ...
        hintLabel:SetColor("Text", ColorUtils.HexToRGBA("FFAAAAAA"))
    end
...
function ContextMenuClass:AddItems(items, isFocus)
    ...
    for _, item in ipairs(items) do
        ...
        if item.HotKey and not self.hotKeySubs[item.Label] then
            self.hotKeySubs[item.Label] = InputEvents.SubscribeKeyAndMouse(function (e)
...
function ImguiElements.AddContextMenu(parent, title)
    local group = parent:AddGroup("ContextMenuGroup##" .. (title or RBUtils.Uuid_v4())) --[[@as ExtuiGroup]]
    if title and title ~= "" then
        local titleText = group:AddText(title)
        titleText.Font = "Medium"
        titleText:SetStyle("Alpha", 0.8)
        titleText:SetColor("Text", ColorUtils.HexToRGBA("FFAAAAAA"))
        group:AddSeparator()
    end
