...
RegisterNetListener("ManageGizmo", function (channel, data, userID)
    ...
    if data.Type == "Clear" then
        GM:DeleteByUserID(userID)
    elseif data.Type == "Create" then
        GM:CreateGizmo(data.Target, data.GizmoType, data.GizmoSpace, userID, data.Guid)
    elseif data.Type == "Delete" then
        GM:RemoveGizmo(data.Guid, userID)
    elseif data.Type == "Update" then
        GM:UpdateGizmo(data.Target, data.Guid, data.GizmoType, data.GizmoSpace, userID)
    elseif data.Type == "ClearAll" then
        GM:ScanAndDeleteAll()
    end
end)
...
RegisterNetListener(NetChannel.Visualize, function (channel, data, userID)
    ...
    elseif data.Type == "Clear" then
        ...
        for _, entityHandles in ipairs(stack or {}) do
    ...
    if duration <= 0 then
        ...
        if data.Static then return end
        PostTo(userID, NetMessage.Visualization, { Guid = entityHandles })
    else
