...
local function getPresentation(keybind)
    ...
    if keybind.Modifiers then
        for _, mod in pairs(keybind.Modifiers) do
            table.insert(parts, SimplifiedInputCode[mod] or mod)
        end
    end
    table.insert(parts, SimplifiedInputCode[keybind.Key] or keybind.Key)
...
function KeybindMenu:RenderEvent(row, moduleName, eventName, module, registry)
    ...
    local resetButton = AddSelectableButton(resetCell, "Reset", function()
        module:Rebind(eventName, initKeybind.Key, initKeybind.Modifiers or nil)
        keyButton.Label = getPresentation(module:GetKeyByEvent(eventName))
    end)
