...
function ChangeFrames(frames, value, isColor)
...
function ChangeABCDFrames(frames, a, b, c, d)
    for _, frame in ipairs(frames) do
        frame.A = a
        frame.B = b
        frame.C = c
...
function ApplyVisualParams(guid, modifiedParams, retryCnt)
    ...
    if not liveEntity or not liveEntity.Visual then
        if not liveEntity and retryCnt < 1 then
            Timer:After(500, function()
                ApplyVisualParams(guid, modifiedParams, retryCnt+1)
            end)
        else
    ...
    local handlers = {
        Scale = function(value, entity)
            local renderable = GetRenderable(entity, value.DescIndex)
            if renderable then
        ...
        Scalar = function(value, entity)
            local material = GetMaterial(entity, value.DescIndex)
            if material then
        ...
        Light = function(value, entity)
            local component = GetEffectComponent(entity, value.CompIndex)
            ...
            if value.PropertyName == "Appearance.Color" then
                ChangeFrames(property.Frames, value.Value, true)
            elseif type(value.Value) == "table" and #value.Value == 4 then
                for _, keyFrame in ipairs(property.KeyFrames) do
                    ChangeABCDFrames(keyFrame.Frames, value.Value[1], value.Value[2], value.Value[3], value.Value[4])
                end
            elseif type(value.Value) == "number" then
                for _, keyFrame in ipairs(property.KeyFrames) do
                    ChangeFrames(keyFrame.Frames, value.Value)
                end
        ...
        LightEntity = function(value, entity)
            local light = GetLightEntity(entity, value.CompIndex)
            if light then
...
function GetRenderable(entity, descIndex)
...
function GetMaterial(entity, descIndex)
    local renderable = GetRenderable(entity, descIndex)
    return renderable and renderable.ActiveMaterial and renderable.ActiveMaterial.Material
end

function GetEffectComponent(entity, compIndex)
...
function SetMaterialParameters(uuid, descIndex, paramName, value, ptype)
    ...
    --- @type Material?
    local material = GetMaterial(entity, descIndex)
    ...
    if not CheckParamExists(paramName, ptype, material) then
...
function CheckParamExists(paramName, ptype, material)
